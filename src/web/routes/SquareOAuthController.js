...
export class SquareOAuthController {
    ...
    constructor({ config, oauthStateSigner, squareOAuthService, keyVaultService }) {
        this.#config = config;
    ...
    registerRoutes(app) {
        app.get('/square/oauth/start', (req, res) => this.#start(req, res));
    ...
    async #start(req, res) {
        try {
            ...
            const principalId = await this.#oauthStateSigner.verify(link);
            if (!principalId) {
            ...
            const state = await this.#oauthStateSigner.create(principalId);
    ...
    async #callback(req, res) {
        ...
        const principalId = await this.#oauthStateSigner.verify(state);
        if (!principalId || !code) {
